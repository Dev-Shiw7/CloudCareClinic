...
# --------------------------------------------------------------------------
# Dashboard (bonus): a read-only HTML view over the same service layer.
# --------------------------------------------------------------------------

def test_dashboard_renders_patients(client):
    created = client.post("/patients", json=_valid_patient(
        last_name="Dashboardsson")).json()["data"]
    r = client.get("/")
    assert r.status_code == 200
    assert "text/html" in r.headers["content-type"]
    assert "Registered Patients" in r.text
    assert created["last_name"] in r.text


def test_dashboard_honours_the_same_filters(client):
    client.post("/patients", json=_valid_patient(last_name="Filterton"))
    assert "Filterton" in client.get(
        "/", params={"last_name": "Filterton"}).text
    assert "No patients match" in client.get(
        "/", params={"last_name": "NoSuchName"}).text


def test_dashboard_survives_an_unparseable_filter(client):
    """A half-typed date must not 500 the page - the form has to stay usable."""
    r = client.get("/", params={"date_of_birth": "not-a-date"})
    assert r.status_code == 200
    assert "Registered Patients" in r.text


def test_dashboard_escapes_patient_values(client):
    """Values arrive from callers by speech, so they are untrusted input."""
    client.post("/patients", json=_valid_patient(
        address_line_1="<script>alert(1)</script>"))
    r = client.get("/")
